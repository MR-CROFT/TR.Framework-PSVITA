...
public class EnemyController : MonoBehaviour
{
    ...
    {
        anim.SetBool("isDead", true); // Activate death animation

        if (deathSFX != null && audioSource != null)
        {
            audioSource.PlayOneShot(deathSFX); // Play death sound effect
        }

        DeactivateAI(); // Deactivate AI components

        Invoke(nameof(DestroyObject), 15f); // Schedule destruction of object after 15 seconds
    }

    private void DeactivateAI()
    {
        var cabalAI = GetComponent<CabalAI>();
        if (cabalAI != null)
        {
            cabalAI.enabled = false;
        }

        var copParisAI = GetComponent<CopParisAI>();
        if (copParisAI != null)
        {
            copParisAI.enabled = false;
        }

        nav.enabled = false; // Disable NavMeshAgent to stop movement
        charControl.enabled = false; // Disable CharacterController
        stateMachine = null; // Nullify state machine reference
    }
