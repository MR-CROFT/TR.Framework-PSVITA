...
public class Ladder : StateBase<PlayerController>
{
    ...
    public override void Update(PlayerController player)
    {
        ...
        // Limita o movimento vertical com base na posição do jogador em relação à escada
        ...
        if (animState.IsName("LadderIdle"))
        {
            ...
            // Verifica se o jogador está no topo da escada e pressiona a tecla de ação
            else if (IsAtLadderTop(player) && Input.GetKeyDown(player.playerInput.action))
            {
                player.Anim.SetTrigger("LadderTopExit");
                isTransitioning = true;
            }
        }
    }

    // Função para verificar se o jogador está no topo da escada
    private bool IsAtLadderTop(PlayerController player)
    {
        // Considera que o jogador está no topo se sua posição Y for próxima à altura total da escada
        return player.transform.position.y >= currentLadder.transform.position.y + currentLadder.MainCollider.size.y - player.charControl.height;
    }
