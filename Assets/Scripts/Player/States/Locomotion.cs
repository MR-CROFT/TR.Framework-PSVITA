...
public class Locomotion : StateBase<PlayerController>
{
    ...
    public override void Update(PlayerController player)
    {
        ...
        // Modified condition to check for the "Slope" tag
        if (player.GroundAngle > player.charControl.slopeLimit && !isRootMotion && player.GroundHit.collider.CompareTag("Slope"))
        {
        ...
        if (!isRootMotion || !Physics.Raycast(player.transform.position, player.transform.forward, 1f))
    ...
    private bool LookForStepLedges(PlayerController player)
    {
        if (!isRootMotion && (player.Anim.GetCurrentAnimatorStateInfo(0).IsName("Idle") || player.Anim.GetCurrentAnimatorStateInfo(0).IsName("Locomotion")))
        {
            // Detect if there is a platform in front of the player
            ledgeDetected = ledgeDetector.FindPlatformInfront(player.transform.position, player.transform.forward, 2f);

            if (ledgeDetected)
            {
                detectedHeight = ledgeDetector.GrabPoint.y - player.transform.position.y;

                // If the player presses the action button, trigger the animation
                if (Input.GetButtonDown("Action"))
                {
                    // Step can be run over
                    if (detectedHeight < player.charControl.stepOffset)
                    {
                        isRootMotion = false;
                        ledgeDetected = false;
                        return false;
                    ...
                    else
                    {
                        ...
                        if (detectedHeight <= 0.9f)
                            player.Anim.SetTrigger("StepUpQtr");
                        else if (detectedHeight <= 1.5f)
                        ...
                        isRootMotion = true;
                        waitingBool = true;
                        ledgeDetected = false; // Reset ledgeDetected
                        return true;
                    }
                }
            }
